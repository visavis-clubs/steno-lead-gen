# src/score_rank.py
# Normalize metrics, compute intent/contactability/freshness, dedupe, and rank.
# Input:  data/leads_enriched.csv  (from Step 5/6)
...
def intent_score(text):
    ...
    # platform + bot/assistant-y words
...
def make_prospect_name(row):
    # YouTube: author_handle is channel title
    # Reddit: author_handle is "u/<name>"
    h = (row.get("author_handle") or "").strip()
    if h.startswith("u/"): return h[2:]
    return h

def dedupe_key(row):
    # Prefer email; else website domain; else author+platform
...
for r in rows:
    ...
    total = i_score*2 + aud_bucket + c_score + f_score
    ...
    scored.append({
        ...
        "source_excerpt": r.get("excerpt","")[:300],
